...
# ==== Load Models ====
cls_path_model_dir = "./models/classify_model"
...
task_model_dir = "./models/classifyTask_model"
...
input_decor_dir = "./models/inputDecor_model"
...
def process_input(text: str):
    ...
    result = {
        "text": text,
        "intent": [],              # top-level intent คงไว้
        "decorated_inputs": [],    # จะไม่มีฟิลด์ intent ภายใน
        "message": ""
    }
    ...
    for idx in path_labels:
        ...
        result["intent"] = ordered_intents_global

        # >>> ใส่เฉพาะ text + decorated (ไม่มี intent ภายใน)
        result["decorated_inputs"].append({
            "text": text,
